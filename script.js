...
if (typeof document !== "undefined") {
    ...
    function home() {
        ...
        <div><button data-study="${s}">Réviser</button><button class="alt" data-test="${s}">Test de ${QUIZ_LENGTH}</button></div></article>`).join("")}</div>
        ...
        $("#custom").onsubmit = e => { 
            e.preventDefault(); 
            quiz($("#sel").value, +$("#num").value || QUIZ_LENGTH); 
        };
    ...
    app.onclick = e => {
        const t = e.target;
        if (t.dataset.study) {
        study(t.dataset.study);
        }
        else if (t.dataset.test) {
        quiz(t.dataset.test, QUIZ_LENGTH);
        }
        else if ("home" in t.dataset) {
        home();
        }
    };
    home();
